<#
  ...
  (docs/VERIFY-PROTOCOL.md, sections 0 and 2.0).

  Contract:
    * section 0   - every installer run is preceded by "clear the target directory, then assert
                    it really is empty". Every run, not just the first.
    * section 2.0 - clear -> assert empty -> install -> uninstall is one atomic cycle.
    * section 2.1 - a silent run never shows a modal frame: a non-empty target is refused with a
    ...
    * section 2.2 - the HKCU uninstall entry and the two shortcuts the user's installation owns are
      snapshotted before the run and restored after it (DEF-014). They belong to that installation
      alone now - the entry key, the display name and both .lnk names are derived from the
      installation directory (DEF-016) - so a test install must leave them untouched; the snapshot is
...
[CmdletBinding()]
param(
  [string]$Dir        = 'D:\dshab\.tmp-verify\install',
  # Empty = the newest dist\DSH-AB-*-setup.exe built by build\build.ps1.
  [string]$Setup      = '',
  [string]$Iss        = 'D:\dshab\installer\dsh-ab.iss',
  [int]   $TimeoutSec = 900,
  [int]   $RefuseSec  = 300
)
...
# Phase 7 steps 2/3/4: the artifact name, the payload record and the versions compiled
...
# cost the user their shortcuts (DEF-014). Installations have an identity of their own now
# (DEF-016), so this run has to leave all three exactly as it found them. They are snapshotted before
...
$ShortcutPaths = @(
  (Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\DSH-AB\DSH-AB.lnk'),
  (Join-Path ([Environment]::GetFolderPath('DesktopDirectory')) 'DSH-AB.lnk'))
...
# Clear-AndAssertEmpty is section 0 of the protocol, in code.
...
# ---- static gate: VERIFY-PROTOCOL section 2.1 -----------------------------------------------
...
if ($bare.Count -gt 0) {
  ...
  throw ("安装器脚本里还有未被抑制的 MsgBox（VERIFY-PROTOCOL 2.1）：" + $NL + $where)
}

# ---- static gate: DEF-002, the installer must not mitigate its own children ------------------
...
# That is exactly the first start after an install. See DEFECTS.md (DEF-002).
if (-not (Select-String -LiteralPath $Iss -Pattern '^\s*RedirectionGuard\s*=\s*no\s*$')) {
  throw "安装器脚本没有 RedirectionGuard=no（DEF-002：Setup 会给 [Run] 子进程套上重定向信任缓解，dsh 的 junction 会被拒绝）"
}

# ---- static gate: the installer and its uninstaller are Chinese (phase 5) ---------------------
...
# unreachable since the AppMutex went away (DEF-015, gate below). A silent run shows no UI at
...
if (-not (Select-String -LiteralPath $Iss -Pattern 'MessagesFile:\s*"languages\\ChineseSimplified\.isl"')) {
  throw "安装器脚本没有指向随仓库的中文语言文件（阶段 5）"
}
if (-not (Test-Path -LiteralPath $isl)) { throw "中文语言文件不存在：$isl" }

# ---- static gate: phase 7 steps 3 and 4 -------------------------------------------------------
...
if (-not (Select-String -LiteralPath $Iss -Pattern '^\s*OutputBaseFilename=\{#AppName\}-\{#DshabVersion\}-\{#DshVersion\}-setup\s*$')) {
  throw "安装包产物名不是 <产品名>-<dshab版本>-<dsh版本>-setup（阶段 7 第 3 步；产品名是 {#AppName}，正式构建即 DSH-AB）"
}
if (-not (Select-String -LiteralPath $Iss -Pattern '(?m)^\s*WelcomeLabel2=.*\{#DshVersion\}')) {
  throw "欢迎页没有显示内置 dsh 版本（阶段 7 第 4 步）"
}
foreach ($label in @('FinishedLabel', 'FinishedLabelNoIcons')) {
  $line = Select-String -LiteralPath $Iss -Pattern "^\s*$label=.*\{#DshVersion\}"
  if (-not $line) { throw "完成页的 $label 没有显示内置 dsh 版本（阶段 7 第 4 步）" }
}

# ---- static gate: a second installation must stay possible (DEF-015) -------------------------
...
if (Select-String -LiteralPath $Iss -Pattern '^\s*AppMutex\s*=') {
  throw "安装器脚本又有了 AppMutex（DEF-015：它会拒绝装第二份，也会在用户实例运行时拒绝安装）"
}

# ---- static gate: every installation keeps its own identity (DEF-016) -------------------------
...
# (SPEC-multi-install-identity.md). The identity is per installation now, derived by [Code] from the
...
if (-not (Select-String -LiteralPath $Iss -Pattern '^\s*CreateUninstallRegKey\s*=\s*no\s*$')) {
  throw '安装器脚本又让 Inno 自建卸载登记项了（DEF-016：一个编译期 AppId 无法描述多份安装，登记项必须由 [Code] 按安装根派生）'
}
if (-not (Select-String -LiteralPath $Iss -Pattern '^\s*UsePreviousAppDir\s*=\s*no\s*$')) {
  throw '安装器脚本又「记住上次安装目录」了（DEF-016：非空目录必被拒，记住的目录只会把用户领进墙）'
}
# P2/P3 把名字与位置都交给 [Code] 算（InstallIconPath / DesktopIconPath），所以这两行的 Name 由
...
foreach ($prefix in @('Name: "{userprograms}\', 'Name: "{autodesktop}\')) {
  ...
  if ($hit.Count -ne 1) { throw "找不到唯一的 [Icons] 入口（$prefix）：$($hit.Count) 条（DEF-016）" }
  if (-not $hit[0].Line.Contains('{code:')) {
    throw "快捷方式名又写死了（DEF-016：两份安装会互相覆盖）：$($hit[0].Line.Trim())"
  }
}
...
# ---- static gate: the installer does no git at all (B1) ---------------------------------------
...
Write-Host '[1/4] 静默安装 + 静默卸载（D15：默认保留用户数据）'
...
foreach ($f in @(
  'DSH_AB.exe', 'dsh-ab.toml', 'README.md', 'LICENSES.txt', '.gitignore',
  'slot-a\node\node.exe', 'slot-a\app\node_modules\@deepseek-ai\dsh\lib\bin.js',
  'slot-a\data\skills\dsh-self-maintenance\SKILL.md', 'docs\PLUGINS.md', 'docs\TODO.md',
  'runtime\git\cmd\git.exe', 'state', 'logs')) {
  Assert-Exists (Join-Path $Dir $f)
}

# ---- A1/A2-1: no AGENTS.md, and no git at all -------------------------------------------------
...
foreach ($f in @(
  ...
  if (Test-Path -LiteralPath (Join-Path $Dir $f)) {
    throw "不该存在的东西出现了：$f（A2-1：安装期不再放 AGENTS.md，也不做任何 git 操作）"
  }
}

# ---- B1: slot-b ships empty -------------------------------------------------------------------
...
if (-not (Test-Path -LiteralPath (Join-Path $Dir 'slot-b'))) { throw '缺少空的 slot-b 目录（B1）' }
$slotB = @(Get-ChildItem -LiteralPath (Join-Path $Dir 'slot-b') -Force)
if ($slotB.Count -ne 0) { throw "slot-b 必须是空目录，实际有 $($slotB.Count) 项" }
Write-Host '      slot-b 存在且为空（B1）'

# ---- A2-1/D1: the installation path is baked in at install time -------------------------------
...
# UTF-8 body on the way through the installer (DEF-011's files are Chinese).
$baked = @(
...
# before the last Go build silently ships an older entry point (that is how DEF-001 survived
...
# what the installed exe reports about itself must be the same pair (phase 7 steps 2/3).
...
# D12 used to be "the installer creates a baseline repository"; the installer does no git at all
...
# D8/D8b: the port asked for on the command line has to be the port in the installed config. The
...
# ---- 应用和功能里的名字：不带版本号，括号在名字里（SPEC-round3 §5.5/§5.6）---------------------
...
# regression §5.5 removed ('DSH-AB 0.1.0 (DSH-AB 3190)'). Which of the three shapes appears depends on
...
# A marker in the user data must survive the silent uninstall (D15 default = keep).
$marker = Join-Path $Dir 'slot-a\data\user-session.txt'
Set-Content -LiteralPath $marker -Value 'keep me' -Encoding ascii

# ---- P1 与 DEF-2 的守卫：本安装根下的进程、条目里记的快捷方式路径 -------------------------
...
# 替身用随包 node 本体：进程名 node.exe、可执行文件路径在本安装根之下，正是 P1 要结束的那一类。它是
# 静默卸载里唯一能证明 P1 真生效的东西——不结束它，紧接着的删除就会失败，目录也删不干净。
...
if (-not (Get-Process -Id $standIn.Id -ErrorAction SilentlyContinue)) {
  throw "替身进程没有起来（PID $($standIn.Id)），这一轮证明不了 P1"
}
...
try {
  ...
  if (Get-Process -Id $standIn.Id -ErrorAction SilentlyContinue) {
    throw "静默卸载没有结束本安装根下的进程（PID $($standIn.Id) 还在）：P1 的那段 PowerShell 又没跑成"
  }
  foreach ($p in $recordedLinks) {
    if (Test-Path -LiteralPath $p) {
      throw "静默卸载没有删掉自己那条快捷方式：$p（DEF-2：要按条目里记的真实路径删）"
    }
  }
  if (Test-Path -LiteralPath (Join-Path $Dir 'DSH_AB.exe')) { throw '卸载后 DSH_AB.exe 还在' }
  if (-not (Test-Path -LiteralPath $marker)) { throw '静默卸载删掉了用户数据（D15 要求默认保留）' }
} finally {
...
Write-Host '      卸载完成：本安装根下的进程已结束、自己的快捷方式已删除、用户数据保留（D15）'

# ---- scenario 2: a non-empty target must be refused, silently, non-zero, without blocking ----
...
Write-Host '[2/4] 非空目录 + 静默参数：必须立即以非 0 退出码结束，不弹框'
Clear-Dir $Dir
...
# ---- scenario 3: an occupied port must be refused, silently, non-zero, without writing ---------
...
Write-Host '[3/4] 生产端口被占用 + 静默参数：必须被查，且必须立即以非 0 退出码结束、目标目录不留文件'
...
# ---- scenario 4: nothing of ours is left behind ----------------------------------------------
Write-Host '[4/4] 清理：本次运行不得在工作区外留下安装痕迹'
Clear-Dir $Dir
foreach ($p in $Outside) {
  if ($Preexisting[$p]) {
