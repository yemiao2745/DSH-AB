; dsh-ab.iss - DSH-AB installer (decision D1).
;
...
;   * refuses to install anywhere that is not an empty folder (D14: never overwrite, never merge),
...
;     and its own desktop shortcut, named by InstallName (SPEC-naming-final.md §1: DSH-AB while it is
...
;   * asks for the production port and writes it into dsh-ab.toml (D8/D8b); an unattended install
;     passes /PRODUCTION= instead of answering the page
;   * installs slot-a and creates an empty slot-b: the copy target for the first change (B1)
;   * does no git operation at all: the AI that maintains the installation runs git itself
;   * on uninstall asks about the user data and keeps it by default (D15)
...
; deleted that entry for both (docs\dev\SPEC-multi-install-identity.md). Two things follow:
...
; two strings never reach the screen (O3). Readiness is not affected either way:
...
OutputDir=..\dist
; Phase 7 step 3: DSH-AB-<dshab version>-<dsh version>-setup.exe（测试构建是
...
; after an install died while every later start worked (DEF-002).
RedirectionGuard=no
...
; The two ledgers the AI maintains (D1); the installer bakes the installation path into them.
...
Source: "{#Payload}\slot-a\*"; DestDir: "{app}\slot-a"; Flags: ignoreversion recursesubdirs createallsubdirs
Source: "{#Payload}\runtime\*"; DestDir: "{app}\runtime"; Flags: ignoreversion recursesubdirs createallsubdirs

[Dirs]
Name: "{app}\state"
Name: "{app}\logs"
; Empty on purpose (B1). An empty slot still counts as "not installed": slotInstalled() requires
; node\node.exe and the dsh entry together, so the tray refuses to switch to it.
Name: "{app}\slot-b"

[Icons]
; 名字与位置都由 P2/P3 的规则算出来（InstallIconPath / DesktopIconPath 只给叶子名，根由
...
ButtonYes=是(&Y)
ButtonNo=否(&N)
; Phase 7 step 4: the finished page names the built-in dsh version too. The wording of
; FinishedLabel/FinishedLabelNoIcons is the language file's own (O7: no sentence is
...
// (VERIFY-PROTOCOL 2.1): a script MsgBox ignores /SUPPRESSMSGBOXES and would block an unattended
...
begin
  ...
  begin
    ...
    begin
      // SuppressibleMsgBox, not MsgBox (VERIFY-PROTOCOL 2.1): a script MsgBox ignores
...
// {{DSH_AB_ROOT}} placeholder the payload was built with (A2-1). The AI is told the path in one
...
// (docs\dev\SPEC-multi-install-identity.md). Every per-installation value is derived from the
...
// ---- P2/P3: the name this installation shows and where its shortcut lives ----------------------
// 名字规则（SPEC-naming-final.md §1）：名字里不出现安装目录的名字，也不看默认目录。
//   * 本机只有这一份，且端口可用           → DSH-AB
//   * 本机有别的份，本份端口可用且不冲突   → DSH-AB (<ports.production>)
//   * 端口不可用，或与别的份的端口相同     → DSH-AB (<tag>)（tag = 安装根 SHA-256 前 12 位，与卸载键同源）
// 括号属于名字本身（SPEC-round3 §5.6）：各显示面直接用这个名字，谁都不再补括号。
// 「冲突」= 本份的 ports.production 与另一份安装的生产端口相同。这一条是必须的：
// P3 把共存的两份放进同一个 DSH-AB 文件夹，名字若相同，后装的那份会覆盖先装的那份的快捷方式，
// 而「绝不替别的安装改快捷方式」不允许这种事。
//
// 这里算的是安装那一刻的名字。DSH-AB 启动时用同一套规则再算一次并重写自己的条目与快捷方式，所以
// 以后改 toml 里的端口，名字会跟着变（P2 的「启动时对齐」）；名字真的退化到 tag 时，那一次启动还会
...
// IsOwnProductEntry 是裁决 B 的扫描口径，必须与 src\dsh-ab\naming.go 的 isDshAbEntry 逐字同规则：
...
// production port, so neither can tell the other apart in a 开始菜单 folder (SPEC-naming-final.md §1).
...
// SPEC-naming-final.md §1 + SPEC-round3-testport-and-testbuild.md §5.6: the only DSH-AB on the
...
// P3 的两个落点只给叶子名：目录由 [Icons] 的 Name 前缀 {userprograms} /
...
begin
  ...
  // 括号（括号已经在 InstallName 的返回值里）。SPEC-round3 §5.5/§5.6。
  ...
  // <Programs>\{#AppName}\ 里的那一条搬出来（SPEC-naming-final.md §3）。不能靠读 .lnk 认领——
...
// deleted exe (2026-09-22 真机验证 DEF-2). 旧版留下的 <Programs>\{#AppName}\ 空文件夹跟着那条链接一起
...
// the user must not be sent after something that is no longer on disk (A4-3).
...
// directories survive, which is what the uninstall prompt defaults to (D15).
//
...
// the user (A4-3). The kept data directories are never reported: they are kept on purpose.
...
// ---- uninstall ---------------------------------------------------------------------------------
// Three things the uninstaller has to say, and the old one said none of them (A4).
//
...
//
// ---- P1: ending this installation's own processes before the uninstall -------------------------
...
// All three go through SuppressibleMsgBox (VERIFY-PROTOCOL 2.1): a silent uninstall answers them
...
begin
  ...
  begin
    // P1: 正在运行的本安装进程会被问一次，而不是只被提醒。默认按钮是「是」，所以静默
    // 卸载（/SUPPRESSMSGBOXES）自己答「是」，也就真的能把文件删干净；选「否」照旧往下走，A4 的
    // 残留清单会告诉用户还剩什么。
    ...
    // Keep by default (D15). SuppressibleMsgBox (VERIFY-PROTOCOL 2.1) answers itself with the
    // default button — IDNO, keep the data — so a silent uninstall never blocks and never deletes.
    DeleteUserData := False;
    if SuppressibleMsgBox('要同时删除 DSH-AB 的用户数据吗？' + #13#10 + #13#10 +
