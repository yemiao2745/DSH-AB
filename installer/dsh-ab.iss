...
;   * on uninstall asks about the user data and keeps it by default
...
; 载荷根（build\mkpayload.ps1 组装出来的那棵树）。默认是仓库里的 payload\；build\build.ps1 会用一个
; 空闲盘符把它接短了再传进来（/DPayload=X:\），原因见那一处：ISCC 读不了超过 MAX_PATH 的**源**路径，
; 而 npm 的嵌套 node_modules 会造出这样一条路径（实测 0.1.5-alpha.2 的载荷里有一条 261 字符的），
; 编译会直接以「系统找不到指定的路径」中止。接短之后源路径前缀只有三四个字符，与仓库位置无关。
#ifndef Payload
