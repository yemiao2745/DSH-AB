...
// TestRunnerKeepsChildOutputRegardlessOfLogLevel is DEF-002's diagnostic floor:
...
// TestWaitPortLiveStopsWhenTheChildDies: the observed DEF-002 shape is a child
...
// the code-level form of requirement section 6 (single popup mechanism).
...
// failure popup: "the process is gone" alone was the whole of DEF-002's story.
...
// --- D23: a failed start is always told, and told once -------------------------
...
// TestStartFailurePopupIsUnconditional (D23): health.popup_on_process_dead
...
// promise the old code could not keep (DEF-002).
func TestStartFailurePopupIsUnconditional(t *testing.T) {
	...
	a.flushPopups() // the report only queues; the lock owner shows it (DEF-006)
...
// TestPortFailureCauseNamesThePortThatWasAlreadyTaken (A2): a port that already
...
// TestStartFailureIsLoggedEvenWhenTheThrottleSilencesThePopup (D23): the popup is
...
// TestStartFailureDoesNotShowThePreviousChildsOutput is DEF-009. The preflight
...
func TestStartFailureDoesNotShowThePreviousChildsOutput(t *testing.T) {
	...
	if strings.Contains(text, "stale-from-the-previous-child") {
		t.Fatalf("the popup shows the previous child's dying words (DEF-009): %s", text)
	}
...
// TestStartupFailureTextSaysWhatToDoNext (A6): the popup used to end at the log
...
// (no restart can free that one - only another port can, A2).
