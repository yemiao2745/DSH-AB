...
// top of the death: one fault, one popup.
...
// something to show when logs.level discards child output.
...
type Runner struct {
	...
	// so a start that fails can explain itself.
	//
	...
	// is deliberately empty so it cannot be mistaken for one.
	tail     *lineTail
	exitNote string
	launched bool
}
...
func (r *Runner) commandLine(slotRoot string, port int) (string, []string, error) {
	...
	// port probe says the instance is really answering.
