...
// P2/P3: 这份安装在「应用和功能」、开始菜单和状态弹窗标题上叫什么名字，以及它的快捷方式放在哪。
//
// 名字规则（SPEC-naming-final.md §1 + SPEC-round3-testport-and-testbuild.md §5.6）：
...
// 测试构建（builduild.ps1 -TestProduct）把 appName 换掉，三种形态同样多一个「test」：
...
// production port, so neither can tell the other apart in a 开始菜单 folder (SPEC-naming-final §1).
...
// included.
//
// Ruling B fixes what counts as one, and installer/dsh-ab.iss's OtherInstallDirs applies the
// same rule verbatim: the entry has an InstallLocation, and either its DisplayName starts with
...
// build with a different one (documents/SPEC-multi-install-identity.md).
...
// guessed name could take another installation's shortcut - which §3 of SPEC-naming-final.md
// forbids. The recorded path is exact, and no other installation's file can ever be named by it.
...
// version number and the parentheses are already part of it (SPEC-round3 §5.5/§5.6). The installer
