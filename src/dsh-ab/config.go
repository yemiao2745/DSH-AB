...
// Config mirrors dsh-ab.toml (the file shipped as build\templates\dsh-ab.toml). Every field drives one
...
// LoadConfig, so the tray can say what is really in force.
...
// to that base; an absolute one is used exactly as written, so
