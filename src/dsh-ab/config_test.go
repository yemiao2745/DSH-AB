...
// TestDefaultConfigMatchesPlan pins every knob of dsh-ab.toml to its documented default:
// build\templates\dsh-ab.toml is the shipped file, and the defaults here have to match it.
...
// TestLoadConfigReportsAFileLevelFailure: one bad key used to discard the whole
...
// TestConfigPopupDoesNotDoubleTheLogLabel: the caller's note already names
...
// TestLoadConfigReportsPerItemRepairs: the per-item fallbacks are counted and
...
// TestLogNoteTellsTheTruthAboutAnUnwritableLogDirectory: every failure popup
