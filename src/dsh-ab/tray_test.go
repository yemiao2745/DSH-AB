...
// TestStartFailurePopupDoesNotHoldTheOperationLock is DEF-006: the popup of a
...
func TestStartFailurePopupDoesNotHoldTheOperationLock(t *testing.T) {
	...
	if !lockFreeAtPopup {
		t.Error("弹窗是在持有 opMu 时显示的（DEF-006）")
	}
	if a.opMu.TryLock() {
		a.opMu.Unlock()
	} else {
		t.Error("弹窗还开着时 opMu 仍被持有：其它托盘操作会一直等下去（DEF-006）")
	}
...
// order stay untouched (requirement section 5).
...
// TestOpenBrowserPopupIsDeferredAndInformational covers the other entry point
// named in DEF-006. Two things are pinned at once: the preflight message of
...
func TestOpenBrowserPopupIsDeferredAndInformational(t *testing.T) {
	...
	case c := <-got:
		...
		if !c.lockFree {
			t.Error("“打开浏览器”的预检提示是在持有 opMu 时显示的（DEF-006）")
		}
		if c.isErr {
...
// TestConfigNoticeNeverBlocksTheStartupPath (O1): a discarded config file must be
// reported, and the report is a modal dialog - so the ready path may only ask for
// it (DEF-007): whoever waits on a dialog holds the whole entry point hostage.
func TestConfigNoticeNeverBlocksTheStartupPath(t *testing.T) {
	...
	case <-time.After(5 * time.Second):
		t.Fatal("configNotice 阻塞了调用者：提示框没被点掉之前，启动路径走不下去（O1 + DEF-007）")
	}
	select {
	case <-shown:
	case <-time.After(5 * time.Second):
...
// TestConfigNoticeShowsTheLogLineOnce (O1-1) walks the same wiring the user's dialog
...
// TestConfigNoticeDoesNotRepeatTheConfigWarning (O1-2): one discarded config file
...
// TestStatusPopupReportsTheConfigState (O1): the status popup says whether this run
...
// TestOpeningTheAppKeepsTheRegistration (PLAN section 6): closing the program and
...
// registration (PLAN section 5 item 2).
...
// TestMenuOrderIsInformationFirst (user 2026-09-19, P1): the tray lists what the
...
// TestStatusPopupReportsWhichCopyIsTalking (B1): two installations run side by
...
// TestTrayTooltipCarriesStateAndPort (P4): 只有一份时也要显示端口，于是 tooltip
...
// TestStatusPopupUsesTheMenuLabel (user 2026-09-19): menu item 2 and the popup it
...
func TestStatusPopupUsesTheMenuLabel(t *testing.T) {
	...
	// P4: 标题以本安装的名字开头，后面才是菜单文字，两份共存的弹窗不再长得一样。
