...
type App struct {
	root string
	cfg  *Config
	// name is what this installation is called on screen (P2): "DSH-AB" while it is
	...
	// so a config the user cannot see is no longer silently ignored (O1).
	...
	// it, and isErr picks the icon. NewApp sets the system MessageBox (DEF-005 owner
	// included) and tests replace it, which is the only way to pin the "one fault,
	...
	// lock is held freezes every other tray action behind it (DEF-006: "重启"
...
func (a *App) onReady() {
	...
	// follows may be a dialog the user has to dismiss first (DEF-007).
	go a.serve()
	go a.launch(true)
	go a.healthLoop()
	// onReady only asks for the config notice; the dialog waits on a goroutine of
	// its own, so nothing here can hold the entry point hostage (DEF-007, O1).
	a.configNotice()
...
// addMenuItems registers the six fixed items through add, in the order the plan
// fixes (2026-09-19, P1 "information first": what can be looked at, then what
...
// fixed by requirement section 5. The status popup keeps its own lines, so the
...
func (a *App) launch(initial bool) {
	...
	a.opMu.Unlock()
	// Never show a dialog under the lock (DEF-006): everything this start wants to
...
func (a *App) launchLocked(initial bool) {
	...
	// the app performs when it opens, and that one keeps it (PLAN section 6).
	...
	// A2: whether the port already answered *before* this start is what turns a bare
	...
	// popup, queued and shown after the lock (DEF-006).
...
// taken" when it looks at the port before starting anything (A2).
...
// one port failure that explains itself (A2). It changes the wording only.
...
type startFailure struct {
	...
	cause string
	// tail and launched belong to the child of *this* attempt (DEF-009).
	tail     []string
	launched bool
	// exited says that child is gone; with launched it is still alive.
	exited bool
	// busyBefore: the port already answered before this start began (A2).
	busyBefore bool
...
// shows is never a lie (DEF-002) — and it never sets ready, which keeps the
// health loop from describing the same failure a second time.
//
// D23: this popup is unconditional — health.popup_on_process_dead does not
...
func (a *App) reportStartFailure(f startFailure) {
	...
	case !launched:
		...
		// the tail belongs to the child the restart just stopped (DEF-009).
...
// (A6). It follows what the attempt really did, and a port that was already taken
...
// (P4: 只有一份也要显示端口). The port is what tells two DSH-AB icons apart
...
// it is the truth about this run's logging (A5): a log directory that cannot be
...
// record the message instead of showing it (DEF-006).
...
// show, and the old child's must not be passed off as them (DEF-009).
...
// after releasing the lock (DEF-006).
...
// (P4: two copies run side by side and their popups look alike), then the item's own
...
// opening the log (O1). It is pure so those lines can be pinned without a desktop.
func (a *App) statusText() string {
	...
	fmt.Fprintf(&b, "\n日志级别：%s", a.lg.Level())
	// B1: two installations run side by side and their popups look alike, so the
...
// two popups (PLAN section 7 item 4).
const (
...
// whether or not the user ever dismisses the notice (DEF-007 shape, O1 requirement 2 -
// the notice must not block startup).
...
func (a *App) showConfigNotice() {
	...
	// the bare title again put two WARNs in the log for the same fact (O1-2).
...
func (a *App) authenticatedURL(wait time.Duration) (string, error) {
	...
	// OBS-002: the address dsh announces is the only one that works. dsh exchanges
